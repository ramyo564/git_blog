...
지난 글에서, 나는 WSL(Windows Subsystem for Linux) 환경을 통해 Spring 컴파일 시간을 절반으로 줄이는 극적인 성능 향상을 경험했습니다. 환호도 잠시, 곧이어 절망적인 에러 메시지가 터미널을 뒤덮었습니다...    
...
왜냐하면 제가 필요한건 네이티브 환경의 속도 '런타임 버전'이라는 핵심 문제만 해결 되면 충분한데 도커컴포즈로 변경사항이 있을 때마다 빌드를 다시 하는 건 오히려 비용이 더 든다고 생각했기 때문입니다.   

## 3. Mise 도입: 3단계 환경 표준화 플레이북
