...
그래서 작성한 코드를 살펴보니 채팅방에서 메세지를 보낼 때 누가누군지 제대로 식별을 할 수 있는 코드가 없었다.   

또한 채팅방을 개설하게 되면 채팅방을 만든 사람이 승인해준 사람만 들어가야 하는 데 그런 로직이 없었다.  

이 때 당시 개인적으로 컨트롤러 단에서 `@AuthenticationPrincipal` 사용하는 대신에 `JwtTokenExtract` 를 필요한 곳에서 호출해서 사용하고 있어서 이걸 웹소켓에서도 사용 가능하게 좀 바꿔서 확장하면 된다고 생각했다.

### 웹소켓에서의 인증방식

그냥 좀 간단하게 이야기 하자면 기존 방식은 사용자1 이 요청을 보내면 서버에서 응답을 보내고 계속 핑퐁핑퐁을 해야한다.  
...
웹소켓을 연결했다가 끊어졌더라도 예를 들어 인터넷 창을 종료했다가 다시 접속했을 때, 대화내용이 그대로 유지할 수 있도록 하면서 누가 해당 메세지를 언제 보냈는지 기록을 하기 위해서`JwtTokenExtract` 를 사용해주면 된다.   
