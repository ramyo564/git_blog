...
해당 글에서는 이 코드를 static 메서드로 만들어서 로그인 객체가 필요한 요청마다 호출하게 된다면 코드가 반복적으로 사용되기 때문에 비효율적이라고 설명하는 걸 봐었는데...  
...
사실 자기 환경에 맞게 사용하면 된다고 생각하지만, 테스트코드 용이성과 나중에 확장성 및 유지보수를 생각한다면 따로 컴포넌트를 만들어서 사용해도 괜찮다고 생각했다.   

이렇게 만들어둔 컴포넌트는 정말 신기하게도 나중에 확장하게 되었다.  
왜 해당 컴포넌트를 만들었고 어쩌다가 확장하게 되었는지는 더 써보겠다.

## 3. `JwtTokenExtract`

`@AuthenticationPrincipal` 를 사용하지 않고 따로 JwtToken을 사용하게 된 이유는 처음에 `@AuthenticationPrincipal` 의 존재여부를 몰랐었다.  
...
로컬 API 테스트를 확인하고 마무리로 테스트코드를 작성하는데 static 으로 선언된 메서드를 확인 하는게 생각보다 어려웠다.  

인터넷을 뒤져서 찾은 정보로는 static으로 선언된 메서드도 테스트를 할 수는 있지만 안티 패턴이라고 추천되지 않는다는 글들을 많이 봤었다.  

사실 static이든 위에처럼 따로 빼서 쓰는 거든 내가 쓰기 편할려고 만든거라 그냥 static이 아니라 bean으로 만들어서 주입하면 그만이라고 생각을 했다.  
...
## 4. JwtTokenExtract 확장 

조금 간략하게 이야기하자면 팀프로젝트 기간 한 달 동안 채팅을 맡은 팀원 A 가 있었는데 알고보니 아주 큰 문제가 있었다.

그 때 당시 프로젝트 종료일까지는 5일이 남은 상태였는데도 결과물이 계속 나오지 않아서 추궁? 하다가 결국은 문제가 많았던 걸 알게 되었다.

추궁을 시작하게 된 계기도 팀프로젝트가 다 끝날 때 까지 결과물이 나오지 않아서 현재 어디까지 개발 되었고 테스트를 어떻게 하고 있냐고 물어보다가 알게 되었다.   

정말 많은 문제들이 있었지만 `JwtTokenExtract` 에 관련된 부분만 요약하자면 채팅기능을 구현하고 테스트하는데 로그인 기능이 없었다.  
채팅방에서 메세지를 보낼 때 누가누군지 제대로 식별을 할 수 있는 코드가 없었다.  
또한 채팅방을 개설하게 되면 채팅방을 만든 사람이 승인해준 사람만 들어가야하는데 그런 로직이 없었다.  

이때 당시 개인적으로 컨트롤러 단에서 `@AuthenticationPrincipal` 사용하는 대신에 `JwtTokenExtract` 를 필요한 곳에서 호출해서 사용하고 있었다. 그래서 이걸 웹소켓에서도 사용 가능하게 좀 바꿔서 확장하면 된다고 생각했다.

### 웹소켓에서의 인증방식

그냥 좀 간단하게 이야기하자면 기존 방식은 사용자1이 요청을 보내면 서버에서 응답을 보내고 계속 핑퐁핑퐁을 해야한다.  
하지만 웹소켓에서는 처음에 한 번 요청을 보낼 때 연결이 된다면 서버의 응답을 기다릴 필요가 없어진다.   

그래서 컨트롤러단이 아닌 필터단에서 사용자의 유효성 여부를 판단하고 조건이 맞지 않는 애들은 그냥 끊어주면 된다.   
...
컴포넌트로 관리하는 `JwtTokenExtract` 에다가 프론트에서 넘겨받을 json 토큰을 파싱해주는걸 추가해줬다.  
...
웹소켓을 연결했다가 끊어졌더라도 예를 들어 인터넷 창을 종료했다가 다시 접속했을 때, 대화내용이 그대로 유지할 수 있도록 하기 위해서`JwtTokenExtract` 를 사용한다.   
