...
Spring Boot 기반 백엔드 프로젝트에서 **세션 기반 인증**의 한계를 극복하고,   
서버 부담을 최소화하면서 강력한 **보안**을 갖춘 인증 시스템을 구축하기 위해 **JWT(JSON Web Token) 기반 무상태 인증 시스템**으로 전환했습니다.   

특히, 단순히 JWT를 사용하는 것을 넘어 **Refresh Token Rotation**, **HttpOnly 쿠키**, 그리고 **Redis를 활용한 블랙리스트 관리**등 보안과 확장성을 고려한 설계에 초점을 맞췄습니다.   

이 글에서는 그 과정과 배경, 그리고 구현 상세 내용을 공유하고자 합니다.
## UseCase
...
## 1. 왜 Auth 도메인을 따로 분리하였는가? (세션 및 단순 JWT 방식의 한계와 차별점)

기존 프로젝트에서 **세션 기반 인증**은 여러 한계를 가지고 있다고 판단했습니다. 또한, **단순 JWT**만을 사용하는 방식 또한 고려했지만, 최종적으로 **Refresh Token**과 **Access Token** 을 분리하고, **Redis**와 **HttpOnly 쿠키**를 활용하는 방식을 선택하게 된 배경은 다음과 같습니다.
...
## 4. 확장성 설계: DDD 및 SOLID 원칙 기반 설계를 중심으로 

프로젝트의 초기 단계에서는 복잡한 팩토리나 빌더 패턴을 명시적으로 적용하기보다는, DDD(Domain-Driven Design)와 SOLID 원칙에 기반한 명확한 계층 분리와 책임 할당을 통해 확장성을 확보하려고 노력했습니다.

- **도메인 계층의 독립성**: 
	-  `domain` 패키지(예: `auth.domain`)는 비즈니스 로직과 도메인 모델에 집중하고, 특정 프레임워크나 인프라 기술에 의존하지 않도록 설계하려고 노력했습니다. 
	- 이렇게 하면 향후 기술 스택 변경 시 도메인 로직의 재사용성을 높일 수 있다고 생각합니다.
- **인터페이스 기반 의존성**:
	- `AuthService`, `RefreshTokenService`, `JwtProvider` 등 주요 컴포넌트들은 추상화된 인터페이스를 통해 의존성 주입(DI)을 받도록 만들었습니다. 
	- 이를 통해 실제 구현체는 언제든지 교체될 수 있으며, 테스트 용이성을 높일 수 있었습니다. 
	- 예를 들어, `UserRepository` 인터페이스를 통해 다양한 데이터 저장소(RDB, NoSQL 등) 구현체를 쉽게 교체할 수 있습니다.
- **CQRS 패턴 고려**: 
	- 쓰기(Command)와 읽기(Query) 모델을 분리하는 CQRS 패턴을 염두에 두고 설계를 진행했습니다. 
	- 현재는 `AuthService`가 두 가지 역할을 모두 수행하지만, 향후 서비스 규모가 커지면 읽기 전용 서비스와 쓰기 전용 서비스를 분리하여 각기 다른 최적화를 적용할 수 있습니다.
- **Value Object 활용**: 
	- `UserId`, `Cursor` 등 도메인 계층에서 불변(Immutable) Value Object를 적극적으로 활용했습니다. 
	- 이는 도메인 모델의 일관성과 안정성을 보장하며, 복잡한 비즈니스 로직을 더 명확하게 표현하는 데 용이하다고 생각합니다.
- **Factory Pattern 활용:**
	- `User` 객체의 생성 로직(특히 OAuth2 사용자 생성 시)을 캡슐화하여 클라이언트 코드에서 생성 방식에 대한 의존성을 제거하고, 유연한 확장을 가능하게 했습니다.
- **Builder Pattern 활용:**
	- `User` 도메인 객체나 `UserResponse DTO`와 같이 여러 필드를 가진 객체를 생성할 때, 생성자의 파라미터가 많아지는 문제를 해결하고 코드의 가독성을 높였습니다. 특히 불변(Immutable) 객체 생성을 안전하게 할 수 있습니다.
- **Mapper Pattern 활용:** 
	- `User` 도메인 모델과 `UserResponse DTO` 간의 변환을 명확하게 분리하여 계층 간 결합도를 낮추고, 도메인 모델의 변화가 외부에 미치는 영향을 최소화했습니다. 
	- 필요한 데이터만 외부에 노출하고 변환 로직을 중앙 집중화하여 재사용성을 높였습니다.
