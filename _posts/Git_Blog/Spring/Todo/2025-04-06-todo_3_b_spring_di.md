...
## 1. 서론: 그럼왜 유연한 설계가 필요한가? (Why)

백엔드 애플리케이션을 개발하며 **테스트 및 유지보수의 어려움**, 특정 기술에 대한 강한 의존성으로 인한 **유연성 저하** 등의 문제에 직면할 때가 많습니다.   

이러한 문제를 해결하고 코드 재사용성을 높이는 방법에 대해 깊이 고민했습니다.

이 글에서는 `extends`와 `implements`의 기본 개념을 넘어, **의존성 주입(DI)** 을 효과적으로 활용하여 코드의 **결합도를 낮추고**, **테스트 용이성**과 **재사용성**을 극대화하는 방안을 Spring Boot 예시와 함께 다룹니다.   

특히 Repository 계층을 분리하여 **도메인 주도 설계(DDD)** 와 **헥사고날 아키텍처** 를 적용한 경험을 기록해봤습니다.
...
단순히 코드를 작성하는 것을 넘어, **더 나은 아키텍처와 유지보수 가능한 시스템을 만들기 위한 고민**의 시작점이 되었습니다.
